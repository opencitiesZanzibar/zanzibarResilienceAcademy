...
function iconByfeature(feature){
  var icon;
  if (feature.properties.amenity === 'atm') icon = atmIcon;
  else if (feature.properties.amenity === 'bank') icon = bankIcon;
  else if (feature.properties.religion === 'muslim') icon = mosqueIcon;
  else if (feature.properties.amenity === 'hospital') icon = hospitalIcon;
  else if (feature.properties.amenity === 'dispensary') icon = hospitalIcon;
  else if (feature.properties.amenity === 'clinic') icon = hospitalIcon;
  else if (feature.properties.amenity === 'restaurant') icon = restaurantIcon;
  else if (feature.properties.amenity === 'school') icon = schoolIcon;
  else icon = otherIcon;
...
//shops
function zoomToShop(e) {
    map.fitBounds(e.target.getBounds());
}
function onEachShop(feature, layer) {
    layer.bindPopup('Shop:'+ feature.properties.shop);
    layer.on({
        click: zoomToShop   
    });
}
...
//manmade features
function zoomToManMade(e) {
    map.fitBounds(e.target.getBounds());
}
function onEachManMade(feature, layer) {
    layer.bindPopup('Feature:'+ feature.properties.man_made);
    layer.on({
        click: zoomToManMade   
    });
}
...
//DATA
var allAmenities = L.geoJson(amenities, {
...
var allshops = L.geoJson(shops, {
    pointToLayer: function (feature, latlng) {
        return L.circleMarker(latlng, styleShops);
    },
    onEachFeature:onEachShop
});

coords = _.map(shops.features, function(th){ 
        return (th.geometry.coordinates).reverse();
});
var shopsheat = L.heatLayer(coords,{ radius: 10 }).addTo(map);

var manMade = L.geoJson(man_made, {
    pointToLayer: function (feature, latlng) {
        return L.circleMarker(latlng, styleManMade);
    },
    onEachFeature:onEachManMade
});
